...
class AsyncPostgresPoolStateStore:
    ...
    async def _take_idle(
        self, pool_name: str, min_remaining_ttl: timedelta
    ) -> TakeIdleResult:
        async with self._operation("try_take_idle", pool_name):
            ...
            async with self._sessions.begin() as session:
                ...
                if existing is None:
                    ...
                    existing.expires_at = now + timedelta(
                        seconds=state.idle_ttl_seconds
                    )

    async def remove_idle(self, pool_name: str, sandbox_id: str) -> None:
        async with self._operation("remove_idle", pool_name):
            async with self._sessions.begin() as session:
                await session.execute(
    ...
    async def try_acquire_primary_lock(
        ...
        async with self._operation("try_acquire_primary_lock", pool_name):
            async with self._sessions.begin() as session:
                state = await self._locked_state(session, pool_name)
                self._require_active(state)
                now = datetime.now(UTC)
    ...
    async def renew_primary_lock(
        ...
        self._validate_owner_ttl(owner_id, ttl)
        async with self._operation("renew_primary_lock", pool_name):
            async with self._sessions.begin() as session:
                state = await self._locked_state(session, pool_name)
                self._require_active(state)
                now = datetime.now(UTC)
    ...
    async def release_primary_lock(self, pool_name: str, owner_id: str) -> None:
        async with self._operation("release_primary_lock", pool_name):
            async with self._sessions.begin() as session:
                state = await self._locked_state(session, pool_name)
    ...
    async def _reap_idle(
        ...
        async with self._operation("reap_expired_idle", pool_name):
            async with self._sessions.begin() as session:
                threshold = now + min_remaining_ttl
                rows = list(
                    (
    ...
    async def snapshot_counters(self, pool_name: str) -> StoreCounters:
        async with self._operation("snapshot_counters", pool_name):
    ...
    async def snapshot_idle_entries(self, pool_name: str) -> list[IdleEntry]:
        async with self._operation("snapshot_idle_entries", pool_name):
            async with self._sessions() as session:
                rows = (
                    await session.scalars(
                        select(WarmPoolIdleSandbox)
    ...
    async def get_max_idle(self, pool_name: str) -> int | None:
        async with self._operation("get_max_idle", pool_name):
            async with self._sessions.begin() as session:
    ...
    async def get_destroy_state(self, pool_name: str) -> PoolDestroyState:
        async with self._operation("get_destroy_state", pool_name):
            async with self._sessions.begin() as session:
                state = await self._locked_state(session, pool_name)
    ...
    async def begin_destroy(self, pool_name: str, owner_id: str) -> None:
        ...
        async with self._operation("begin_destroy", pool_name):
            async with self._sessions.begin() as session:
                state = await self._locked_state(session, pool_name)
    ...
    async def clear_pool_state(self, pool_name: str) -> None:
        async with self._operation("clear_pool_state", pool_name):
            async with self._sessions.begin() as session:
                state = await self._locked_state(session, pool_name)
                await session.execute(
                    delete(WarmPoolIdleSandbox).where(
        ...
        async with self._operation("mark_destroyed", pool_name):
            async with self._sessions.begin() as session:
                ...
                state.destroy_expires_at = (
                    datetime.now(UTC) + tombstone_ttl
